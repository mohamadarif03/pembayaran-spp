...
@section('content')
    <div class="p-6 lg:p-8 footer-inner mx-auto main-container lg:py-0 py-0 container" x-bind:class="setting.page_layout">
        <div class="flex flex-col mb-8 text-secondary-500 bg-white dark:bg-dark-card shadow rounded">
            ...
            <div class="p-6">
                <form action="/classroom/{{$classroom->id}}" method="POST" enctype="multipart/form-data">
                  @csrf
                    <div class="mb-4">
                        <label class="inline-block mb-2 text-secondary-500 dark:text-white text-sm">Nama Kelas </label>
                        <input type="text" name="name" class="w-full px-4 py-2 text-secondary-500 border dark:bg-dark-card dark:border-secondary-800 rounded outline-none focus:border-primary-500 dark:focus:border-primary-500 focus:shadow dark:text-white text-sm" value="{{ old('name', $classroom->name ) }}" placeholder="Nama Kelas">
                    </div>
